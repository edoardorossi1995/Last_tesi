...
def policy_improv(P, R, gamma, v_pi, index_list):
    ...
    for s in range(S):
        q.fill(0.0)
        for a in range(A):   
            # EDOARDO - aggiunta condizione per evitare improvment in s,a illegali           
            if (s, a) not in index_list:
                p_trans = P[s, :, a]
                q[a] = R[s, a] + gamma*np.dot(p_trans, v_pi)
        pi[s] = np.argmax(q)
    return pi

def policy_iteration(P, R, gamma, v_init, pi_init, index_list, tol=1.0e-6):
    ...
    while True:
        ...
        pi = policy_improv(P, R, gamma, v, index_list)
